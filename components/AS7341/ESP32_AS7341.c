 ...
 *  @param  value
 */
static void write8(ESP32_AS7341 *AS,uint8_t reg, uint8_t value) {
    uint8_t buffer[2] = {reg, value};
    ESP_ERROR_CHECK(i2c_master_write_to_device(AS->i2c_port, AS7341_ADDRESS, buffer, 2, 1000 / portTICK_PERIOD_MS));
}
 ...
 *  @param  value
 */
static void write16(ESP32_AS7341 *AS, uint8_t reg, uint16_t value) {

    uint8_t buffer[3] = {reg, (uint8_t)(value), (uint8_t)(value >> 8)};
    ESP_ERROR_CHECK(i2c_master_write_to_device(AS->i2c_port, AS7341_ADDRESS, buffer, 3, 1000 / portTICK_PERIOD_MS));
}
 ...
 *  @return value
 */
static uint8_t read8(ESP32_AS7341 *AS, uint8_t reg) {
    uint8_t buffer[1] = {reg};
    ESP_ERROR_CHECK(i2c_master_write_to_device(AS->i2c_port, AS7341_ADDRESS, buffer, 1, 1000 / portTICK_PERIOD_MS));
    ESP_ERROR_CHECK(i2c_master_read_from_device(AS->i2c_port, AS7341_ADDRESS, buffer, 1, 1000 / portTICK_PERIOD_MS));
    return buffer[0];
}
 ...
 *  @return value
 */
static uint16_t read16(ESP32_AS7341 *AS, uint8_t reg) {
    uint8_t buffer[2] = {reg, 0};
    ESP_ERROR_CHECK(i2c_master_write_to_device(AS->i2c_port, AS7341_ADDRESS, buffer, 1, 1000 / portTICK_PERIOD_MS));
    ESP_ERROR_CHECK(i2c_master_read_from_device(AS->i2c_port, AS7341_ADDRESS, buffer, 2, 1000 / portTICK_PERIOD_MS));
 ...
 *  @return value
 */
static void read_data(ESP32_AS7341 *AS, uint8_t reg, uint8_t *data, size_t size) {
    uint8_t buffer[1] = {reg};
    ESP_ERROR_CHECK(i2c_master_write_to_device(AS->i2c_port, AS7341_ADDRESS, buffer, 1, 1000 / portTICK_PERIOD_MS));
    ESP_ERROR_CHECK(i2c_master_read_from_device(AS->i2c_port, AS7341_ADDRESS, data, size, 1000 / portTICK_PERIOD_MS));
}
 ...
 */
static void writeBits(ESP32_AS7341 *AS, uint8_t reg, uint8_t mask, uint8_t value) {
    uint8_t tmp = read8(AS, reg);
    tmp &= ~mask;
    tmp |= (value & mask);
    write8(AS, reg, tmp);
}
...
esp_err_t AS_init(ESP32_AS7341 *AS, uint8_t i2c_port) {
    assert(AS->init!=true && ALREADY_INIT);
    AS->i2c_port=i2c_port;
    //Load default values
    ...
    uint8_t ID = read8(AS, AS7341_ID) >> 2;
...
void AS_enable(ESP32_AS7341 *AS){
    assert(AS->init==true && NOT_INIT);
    writeBits(AS, AS7341_ENABLE, 0x01, 0x01);   //PON=1
    AS->enabled=true;
}
...
void AS_disableAll(ESP32_AS7341 *AS) {
    assert(AS->init==true && NOT_INIT);
    writeBits(AS, AS7341_ENABLE, 0x5b, 0x00);  //PON=0, SP_EN=0, W_EN=0, SMUX_EN=0, FD_EN=0
    AS->enabled=0;
}
...
void AS_enableSpectralMeasurement(ESP32_AS7341 *AS, bool enable_measurement) {
    assert(AS->init==true && NOT_INIT);
    writeBits(AS, AS7341_ENABLE, 0x02, (uint8_t)enable_measurement << 1); //SP_EN=enable_measurement
}
...
void AS_setMode(ESP32_AS7341 *AS, as7341_mode_t mode){
    assert(AS->init==true && NOT_INIT);
    writeBits(AS, AS7341_CONFIG, 0x03, (uint8_t)mode);
    AS->mode = mode;
}
...
void AS_setASTEP(ESP32_AS7341 *AS, uint16_t ASTEP){
    assert(AS->init==true && NOT_INIT);
    write16(AS, AS7341_ASTEP_L, ASTEP);
    AS->ASTEP = ASTEP;
}
...
void AS_setGain(ESP32_AS7341 *AS, as7341_gain_t gain){
    assert(AS->init==true && NOT_INIT);
    writeBits(AS, AS7341_CFG1, 0x1F, (uint8_t)gain);
    AS->gain = gain;
}
...
void AS_setAPERS(ESP32_AS7341 *AS, as7341_int_cycle_count_t APERS){
    assert(AS->init==true && NOT_INIT);
    writeBits(AS, AS7341_PERS, 0x0F, (uint8_t)APERS);
    AS->APERS = APERS;
}
...
void AS_setLowThreshold(ESP32_AS7341 *AS, uint16_t threshold){
    assert(AS->init==true && NOT_INIT);
    write16(AS, AS7341_SP_LOW_TH_L, threshold);
    AS->LT = threshold;
}
...
void AS_setHighThreshold(ESP32_AS7341 *AS, uint16_t threshold){
    assert(AS->init==true && NOT_INIT);
    write16(AS, AS7341_SP_HIGH_TH_L, threshold);
    AS->HT = threshold;
}
...
void AS_setThresholdChannel(ESP32_AS7341 *AS, as7341_adc_channel_t channel){
    ...
    writeBits(AS, AS7341_CFG12, 0x07, (uint8_t)channel);
}
...
void AS_setGPIO(ESP32_AS7341 *AS, as7341_gpio_dir_t dir, bool gpio_inverted, bool out_gpio_value){
    ...
    writeBits(AS, AS7341_GPIO2, 0x0F, buffer);
...
void AS_setBank(ESP32_AS7341 *AS, bool sel) {
    assert(AS->init==true && NOT_INIT);
    writeBits(AS, AS7341_CFG0, 0x10, (uint8_t)sel << 4);
    AS->reg_bank=sel;
}
 ...
 */
static void setup_F1F4_Clear_NIR(ESP32_AS7341 *AS) {
    // SMUX Config for F1,F2,F3,F4,NIR,Clear
    write8(AS, 0x00, 0x30); // F3 left set to ADC2
    write8(AS, 0x01, 0x01); // F1 left set to ADC0
    write8(AS, 0x02, 0x00); // Reserved or disabled
    write8(AS, 0x03, 0x00); // F8 left disabled
    write8(AS, 0x04, 0x00); // F6 left disabled
    write8(AS, 0x05, 0x42); // F4 left connected to ADC3/f2 left connected to ADC1
    write8(AS, 0x06, 0x00); // F5 left disbled
    write8(AS, 0x07, 0x00); // F7 left disbled
    write8(AS, 0x08, 0x50); // CLEAR connected to ADC4
    write8(AS, 0x09, 0x00); // F5 right disabled
    write8(AS, 0x0A, 0x00); // F7 right disabled
    write8(AS, 0x0B, 0x00); // Reserved or disabled
    write8(AS, 0x0C, 0x20); // F2 right connected to ADC1
    write8(AS, 0x0D, 0x04); // F4 right connected to ADC3
    write8(AS, 0x0E, 0x00); // F6/F8 right disabled
    write8(AS, 0x0F, 0x30); // F3 right connected to AD2
    write8(AS, 0x10, 0x01); // F1 right connected to AD0
    write8(AS, 0x11, 0x50); // CLEAR right connected to AD4
    write8(AS, 0x12, 0x00); // Reserved or disabled
    write8(AS, 0x13, 0x06); // NIR connected to ADC5
}
 ...
 * See AS7341_EvalSW_Reflection_v1-26-3.zip for explanations
 */
static void setup_F5F8_Clear_NIR(ESP32_AS7341 *AS) {
    // SMUX Config for F5,F6,F7,F8,NIR,Clear
    write8(AS, 0x00, 0x00); // F3 left disable
    write8(AS, 0x01, 0x00); // F1 left disable
    write8(AS, 0x02, 0x00); // reserved/disable
    write8(AS, 0x03, 0x40); // F8 left connected to ADC3
    write8(AS, 0x04, 0x02); // F6 left connected to ADC1
    write8(AS, 0x05, 0x00); // F4/ F2 disabled
    write8(AS, 0x06, 0x10); // F5 left connected to ADC0
    write8(AS, 0x07, 0x03); // F7 left connected to ADC2
    write8(AS, 0x08, 0x50); // CLEAR Connected to ADC4
    write8(AS, 0x09, 0x10); // F5 right connected to ADC0
    write8(AS, 0x0A, 0x03); // F7 right connected to ADC2
    write8(AS, 0x0B, 0x00); // Reserved or disabled
    write8(AS, 0x0C, 0x00); // F2 right disabled
    write8(AS, 0x0D, 0x00); // F4 right disabled
    write8(AS, 0x0E, 0x24); // F8 right connected to ADC2/ F6 right connected to ADC1
    write8(AS, 0x0F, 0x00); // F3 right disabled
    write8(AS, 0x10, 0x00); // F1 right disabled
    write8(AS, 0x11, 0x50); // CLEAR right connected to AD4
    write8(AS, 0x12, 0x00); // Reserved or disabled
    write8(AS, 0x13, 0x06); // NIR connected to ADC5
}

//TODO flicker not implemented yet
// /**
//  * @brief Configure SMUX for flicker detection
//  * See AS7341_EvalSW_Reflection_v1-26-3.zip for explanations
//  */
// static void FDConfig(ESP32_AS7341 *AS) {
//     // SMUX Config for Flicker- register (0x13)left set to ADC6 for flicker
//     // detection
//     write8(AS, 0x00, 0x00);  // disabled
//     write8(AS, 0x01, 0x00);  // disabled
//     write8(AS, 0x02, 0x00);  // reserved/disabled
//     write8(AS, 0x03, 0x00);  // disabled
//     write8(AS, 0x04, 0x00);  // disabled
//     write8(AS, 0x05, 0x00);  // disabled
//     write8(AS, 0x06, 0x00);  // disabled
//     write8(AS, 0x07, 0x00);  // disabled
//     write8(AS, 0x08, 0x00);  // disabled
//     write8(AS, 0x09, 0x00);  // disabled
//     write8(AS, 0x0A, 0x00);  // disabled
//     write8(AS, 0x0B, 0x00);  // Reserved or disabled
//     write8(AS, 0x0C, 0x00);  // disabled
//     write8(AS, 0x0D, 0x00);  // disabled
//     write8(AS, 0x0E, 0x00);  // disabled
//     write8(AS, 0x0F, 0x00);  // disabled
//     write8(AS, 0x10, 0x00);  // disabled
//     write8(AS, 0x11, 0x00);  // disabled
//     write8(AS, 0x12, 0x00);  // Reserved or disabled
//     write8(AS, 0x13, 0x60); // Flicker connected to ADC5 to left of 0x13
// }

static void setSMUXCommand(ESP32_AS7341 *AS, as7341_smux_cmd_t command) {
    writeBits(AS, AS7341_CFG6, 0x18, (uint8_t)(command << 3));
}

static void enableSMUX(ESP32_AS7341 *AS) {

    writeBits(AS, AS7341_ENABLE, 0x10, 0x01 << 4);
    ...
    while (!!((read8(AS, AS7341_ENABLE) & 0x10) >> 4) && count < timeOut) {
...
static void setSMUXLowChannels(ESP32_AS7341 *AS, bool lowChan) {
    AS_enableSpectralMeasurement(AS, false);
    setSMUXCommand(AS, AS7341_SMUX_CMD_WRITE);
    if (lowChan) {
        setup_F1F4_Clear_NIR(AS);
    } else {
        setup_F5F8_Clear_NIR(AS);
    }
    enableSMUX(AS);
}
...
bool AS_getIsDataReady(ESP32_AS7341 *AS) {
    ...
    return !!((read8(AS, AS7341_STATUS2) & 0x40) >> 6);
}
...
void AS_delayForData(ESP32_AS7341 *AS, int waitTime) {
    ...
    if (waitTime > 0) // Wait for that many milliseconds
    {
        ...
        while (!AS_getIsDataReady(AS) && elapsedMillis < waitTime) {
        vTaskDelay(1/portTICK_PERIOD_MS);
        elapsedMillis++;
        }
...
void AS_read_ADC_Channel(ESP32_AS7341 *AS, as7341_adc_channel_t channel) {
    ...
    AS->data[channel]=read16(AS, AS7341_CH0_DATA_L + 2 * channel);
}
...
void AS_readAllChannels(ESP32_AS7341 *AS) {
    assert(AS->init==true && NOT_INIT);
    setSMUXLowChannels(AS, true);             // Configure SMUX to read low channels
    AS_enableSpectralMeasurement(AS, true);   // Start integration
    AS_delayForData(AS, 0);                   // I'll wait for you for all time

    uint8_t readings_buffer[12];
    read_data(AS, AS7341_CH0_DATA_L, readings_buffer, 12);  // Read all 12 bytes
    ...
    setSMUXLowChannels(AS, false);            // Configure SMUX to read high channels
    AS_enableSpectralMeasurement(AS, true);   // Start integration
    AS_delayForData(AS, 0);                   // I'll wait for you for all time

    read_data(AS, AS7341_CH0_DATA_L, readings_buffer, 12);
    // Convert the readings to uint16_t and store in the AS structure
...
/**
 * @brief Fill the data argument with all the readings taken from the AS
 * structure
 *
 *  call `readAllChannels` before reading to update the stored readings
 *
 * @param AS Pointer to the AS7341 structure
 * @param data Array of 12 uint16_t
 * @return void
 */
 ...
 * @return ITIME value
 */
static uint32_t AS_getITIME(ESP32_AS7341 *AS){
    assert(AS->init==true && NOT_INIT);
    uint8_t buffer[3];
    buffer[0]=read8(AS, AS7341_ITIME_L);
    buffer[1]=read8(AS, AS7341_ITIME_M);
    buffer[2]=read8(AS, AS7341_ITIME_H);
...
long AS_getTINT(ESP32_AS7341 *AS) {
    ...
    if(AS->mode==AS7341_MODE_SPM || AS->mode==AS7341_MODE_SYNS){
        ...
        return AS_getITIME(AS) * 2.78 / 1000;
    }
}
